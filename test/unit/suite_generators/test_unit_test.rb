    ...
    class WithInputFilesPatternAttributeAndOutputDirAttributeNotAsFileAndProbeAttributeHavingNoResults <
      ...
      def setup
        ...
        @generator = Trapeze::SuiteGenerators::TestUnit.new(:input_files_pattern => 'foo',
                                                            :output_dir => 'bar',
                                                            :probe => @mock_probe)
        @generator.stubs(:output_dir_file?).returns false
        FileUtils.stubs :rm_rf
        FileUtils.stubs :mkdir_p
        @generator.stubs(:template_or_partial_exist?).returns true
        File.stubs(:open).yields stub_everything('IO')
      ...
      def test_should_call_file_open_with_suite_filename_and_expected_modestring_when_sent_generateEXCLAMATION
        File.expects(:open).with('bar/SUITE.rb', 'w+').yields stub_everything('IO')
        @generator.generate!
      end
      
      def test_should_print_expected_content_to_suite_file_when_sent_generateEXCLAMATION
        File.stubs(:read).returns 'expected SUITE file content'
        mock_io = mock('IO')
        mock_io.expects(:print).with 'expected SUITE file content'
